...
async function handleStart(ctx: Context, walletService: WalletService): Promise<void> {
    ...
    // Rate limiting check
    const lastStart = startCommandCooldown.get(chatId);
    const now = Date.now();
    if (lastStart && (now - lastStart) < START_COOLDOWN_MS) {
        // User is spamming /start, ignore silently
        return;
    }
    startCommandCooldown.set(chatId, now);
    ...
    try {
        if (fs.existsSync(BANNER_PATH)) {
            // Use cached file_id if available to avoid re-uploading
            if (cachedBannerFileId) {
                ...
                // First time: upload from file and cache the file_id
                const result = await ctx.replyWithPhoto(
                    { source: BANNER_PATH },
                    { caption: welcomeMessage, parse_mode: 'Markdown', ...getMainMenuKeyboard(hasWallet, lang) }
                );
                ...
                if (result.photo && result.photo.length > 0) {
                    cachedBannerFileId = result.photo[result.photo.length - 1].file_id;
                    console.log('Banner file_id cached:', cachedBannerFileId);
                }
            }
        } else {
            // Fallback to text-only if banner doesn't exist
        ...
        if (error?.response?.error_code === 429) {
            ...
            // Don't retry immediately, just send text fallback
            try {
                await ctx.reply(welcomeMessage, { parse_mode: 'Markdown', ...getMainMenuKeyboard(hasWallet, lang) });
            } catch (e) {
